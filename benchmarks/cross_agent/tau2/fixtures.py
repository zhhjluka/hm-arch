"""tau2-bench retail and airline domain fixtures (HM-76 / MEM-76).

Offline fixtures mirror tau2-bench task-log structure: multi-turn customer
service trajectories with explicit task success criteria per domain.
"""
...
def retail_fixture() -> SyntheticFixture:
    """Retail domain: order lookup, return initiation, and refund status."""
    ingest = (
        IngestItem(
            ...
            metadata={"domain": "retail", "task": "lookup_order", "status": "completed"},
        ),
        IngestItem(
            ...
            metadata={"domain": "retail", "task": "initiate_return", "status": "completed"},
        ),
        IngestItem(
            ...
            metadata={"domain": "retail", "task": "process_refund", "status": "failed"},
        ),
        IngestItem(
            ...
            metadata={"domain": "retail", "task": "update_shipping_address", "status": "completed"},
        ),
    )
    queries = (
        BenchmarkQuery(
            ...
            metadata={"domain": "retail", "task": "initiate_return"},
        ),
        BenchmarkQuery(
            query_id="retail-q2",
            ...
            metadata={"domain": "retail", "task": "process_refund"},
        ),
        BenchmarkQuery(
            query_id="retail-q3",
            ...
            task_success_criteria="completed",
            metadata={"domain": "retail", "task": "update_shipping_address"},
        ),
    )
...
def airline_fixture() -> SyntheticFixture:
    """Airline domain: booking changes, seat selection, and cancellation."""
    ingest = (
        IngestItem(
            ...
            metadata={"domain": "airline", "task": "change_flight", "status": "completed"},
        ),
        IngestItem(
            ...
            metadata={"domain": "airline", "task": "select_seat", "status": "completed"},
        ),
        IngestItem(
            ...
            metadata={"domain": "airline", "task": "cancel_booking", "status": "failed"},
        ),
        IngestItem(
            ...
            metadata={"domain": "airline", "task": "add_baggage", "status": "completed"},
        ),
    )
    queries = (
        BenchmarkQuery(
            ...
            task_success_criteria="completed",
            metadata={"domain": "airline", "task": "change_flight"},
        ),
        BenchmarkQuery(
            query_id="airline-q2",
            ...
            task_success_criteria="completed",
            metadata={"domain": "airline", "task": "select_seat"},
        ),
        BenchmarkQuery(
            query_id="airline-q3",
            ...
            task_success_criteria="failed",
            metadata={"domain": "airline", "task": "cancel_booking"},
        ),
    )
    return SyntheticFixture(
        family=BenchmarkFamily.TAU2_BENCH,
...
_DOMAIN_FIXTURES: dict[Tau2Domain, SyntheticFixture] = {
    Tau2Domain.RETAIL: retail_fixture(),
    Tau2Domain.AIRLINE: airline_fixture(),
}


def get_tau2_domain_fixture(domain: Tau2Domain) -> SyntheticFixture:
