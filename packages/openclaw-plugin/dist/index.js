...
const plugin = definePluginEntry({
    ...
    register(api) {
        if (hasMemorySlotConflict(api.config)) {
            api.logger.warn(`memory-hm-arch: plugins.slots.memory is not '${HM_ARCH_PLUGIN_ID}'; skipping memory registration`);
            return;
        }
        if (!isActiveMemoryPlugin(api.config)) {
            api.logger.warn(`memory-hm-arch: memory slot is not configured for '${HM_ARCH_PLUGIN_ID}'; plugin loaded but inactive`);
        }
        const captureCursors = new Map();
        const resolveConfig = () => resolvePluginConfig(api.pluginConfig, (input) => api.resolvePath(input));
        const sidecarHolder = {
        ...
        api.registerService({
            ...
            start: async () => {
                ...
                try {
                    ...
                    api.logger.warn?.(`memory-hm-arch: sidecar failed to start (fail-open): ${String(error)}`);
                }
            },
            stop: async () => {
                try {
        ...
        api.on("before_prompt_build", async (event) => await runBeforePromptBuild({
            config: resolveConfig(),
            sidecar: sidecarHolder.current,
            logger: api.logger,
            captureCursors,
        }, event), { timeoutMs: resolveConfig().requestTimeoutMs });
