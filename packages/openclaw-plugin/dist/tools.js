...
export function createMemoryTools(options) {
    const getSidecar = () => "current" in options.sidecar ? options.sidecar.current : options.sidecar;
    const recallTool = {
        name: "memory_recall",
        label: "Memory Recall",
        description: "Search HM-Arch durable memory for relevant facts, preferences, and prior decisions.",
        parameters: {
            type: "object",
            properties: {
        ...
        async execute(_toolCallId, params) {
            ...
            const limit = typeof raw.limit === "number" && Number.isInteger(raw.limit) && raw.limit > 0
                ? raw.limit
                : config.topK;
            try {
                ...
                if (!response.ok) {
                    options.logger.warn?.(`memory-hm-arch: memory_recall failed: ${response.error?.message ?? "unknown error"}`);
                    return {
                        content: [
                            {
                                type: "text",
    ...
    const storeTool = {
        ...
        async execute(_toolCallId, params) {
            ...
            const importance = typeof raw.importance === "number" && raw.importance >= 0 && raw.importance <= 1
                ? raw.importance
                : 0.7;
            try {
                ...
                if (!response.ok || response.result.recorded !== true) {
                    options.logger.warn?.(`memory-hm-arch: memory_store failed: ${response.error?.message ?? "unknown error"}`);
                    return {
    ...
    const forgetTool = {
        ...
        async execute(_toolCallId, params) {
            const raw = params;
            const memoryId = (typeof raw.memoryId === "string" ? raw.memoryId : "") ||
            ...
            try {
                ...
                const forgotten = typeof response.result.forgotten_count === "number"
