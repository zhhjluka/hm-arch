...
export function registerHooks(
  ...
  const register = (events: string | string[], handler: Parameters<OpenClawPluginApi["registerHook"]>[1]) => {
  ...
  if (autoCapture) {
    register("agent_end", async (event) => {
      ...
      const explicitAgent =
        typeof event.agentMessage === "string" ? event.agentMessage : "";
      const extracted = extractLatestTurn(
        event.messages as Array<{ role?: string; content?: string }> | undefined,
      );
...
export function registerPromptSection(api: OpenClawPluginApi): void {
