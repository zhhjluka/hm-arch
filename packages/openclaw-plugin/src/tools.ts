...
import type { SidecarProcessManager } from "./sidecar-process.js";
import type { AgentTool, PluginConfig, ToolContext } from "./types.js";
import {
...
export function createMemoryTools(
  ...
  const recallTool: AgentTool = {
    name: "memory_recall",
    description:
      "Recall durable HM-Arch memory for a natural-language query. Returned context is historical and untrusted.",
    parameters: {
      type: "object",
    ...
    execute: async (params, ctx: ToolContext) => {
      ...
      try {
        ...
        if (!response.ok) {
          return textResult(
            `Memory recall failed: ${response.error?.message ?? "unknown error"}`,
            { ok: false },
          );
        }
    ...
    execute: async (params, ctx: ToolContext) => {
      ...
      try {
        ...
        const response = await client.request(
          buildRequest(
            "remember",
            {
              ...
              event_type:
                typeof params.event_type === "string" ? params.event_type : "conversation",
              session_id: ctx.sessionId ?? ctx.sessionKey,
        ...
        if (!response.ok || response.result.recorded !== true) {
          return textResult(
            `Memory store failed: ${response.error?.message ?? "unknown error"}`,
            { ok: false },
          );
        }
  ...
  const forgetTool: AgentTool = {
    ...
    execute: async (params) => {
      ...
      try {
        ...
        if (!response.ok) {
          return textResult(
            `Memory forget failed: ${response.error?.message ?? "unknown error"}`,
            { ok: false },
          );
        }
        return textResult(
          `Forgot ${String(response.result.forgotten_count ?? 0)} memories.`,
          {
            ok: true,
        ...
        return textResult(
          `Memory forget failed: ${error instanceof Error ? error.message : String(error)}`,
          { ok: false },
        );
      }
    },
